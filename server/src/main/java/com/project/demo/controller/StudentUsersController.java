...
public class StudentUsersController extends BaseController<StudentUsers, StudentUsersService> {
    ...
    public Map<String, Object> add(HttpServletRequest request) throws IOException {
        ...
        Map<String, String> mapstudent_name = new HashMap<>();
        mapstudent_name.put("student_name",String.valueOf(paramMap.get("student_name")));
        List liststudent_name = service.selectBaseList(service.select(mapstudent_name, new HashMap<>()));
        if (liststudent_name.size()>0){
            return error(30000, "字段学生姓名内容不能重复");
        }
