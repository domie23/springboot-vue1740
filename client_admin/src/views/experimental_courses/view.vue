	...
	<el-main class="bg edit_wrap">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
				...
				<el-form-item label="实验室图" prop="laboratory_diagram">
						...
						:show-file-list="false" v-if="user_group === '管理员' || (form['experimental_courses_id'] && $check_field('set','laboratory_diagram')) || (!form['experimental_courses_id'] && $check_field('add','laboratory_diagram'))">
				...
				<el-form-item label="实验室编号" prop="laboratory_number">
							  ...
							  v-if="user_group === '管理员' || (form['experimental_courses_id'] && $check_field('set','laboratory_number')) || (!form['experimental_courses_id'] && $check_field('add','laboratory_number'))" :disabled="disabledObj['laboratory_number_isDisabled']"></el-input>
				...
				<el-form-item label="实验室名" prop="laboratory_name">
							  ...
							  v-if="user_group === '管理员' || (form['experimental_courses_id'] && $check_field('set','laboratory_name')) || (!form['experimental_courses_id'] && $check_field('add','laboratory_name'))" :disabled="disabledObj['laboratory_name_isDisabled']"></el-input>
				...
				<el-form-item label="位置" prop="position">
							  ...
							  v-if="user_group === '管理员' || (form['experimental_courses_id'] && $check_field('set','position')) || (!form['experimental_courses_id'] && $check_field('add','position'))" :disabled="disabledObj['position_isDisabled']"></el-input>
				...
				<el-form-item label="容纳人数" prop="capacity">
							  ...
							  v-if="user_group === '管理员' || (form['experimental_courses_id'] && $check_field('set','capacity')) || (!form['experimental_courses_id'] && $check_field('add','capacity'))" :disabled="disabledObj['capacity_isDisabled']"></el-input>
				...
				<el-form-item label="课程名称" prop="course_name">
							  ...
							  v-if="user_group === '管理员' || (form['experimental_courses_id'] && $check_field('set','course_name')) || (!form['experimental_courses_id'] && $check_field('add','course_name'))" :disabled="disabledObj['course_name_isDisabled']"></el-input>
				...
				<el-form-item label="课程类别" prop="course_category">
								<el-select id="course_category" v-model="form['course_category']"
						v-if="user_group === '管理员' || (form['experimental_courses_id'] && $check_field('set','course_category')) || (!form['experimental_courses_id'] && $check_field('add','course_category'))">
						<el-option v-for="o in list_course_category" :key="o" :label="o" :value="o">
						</el-option>
					</el-select>
				...
				<el-form-item label="任课老师" prop="lecturer">
													<el-select v-if="user_group === '管理员' || (form['experimental_courses_id'] && $check_field('set','lecturer')) || (!form['experimental_courses_id'] && $check_field('add','lecturer'))" id="lecturer" v-model="form['lecturer']" :disabled="disabledObj['lecturer_isDisabled']">
				...
				<el-form-item label="上课时间" prop="class_time">
							  ...
							  v-if="user_group === '管理员' || (form['experimental_courses_id'] && $check_field('set','class_time')) || (!form['experimental_courses_id'] && $check_field('add','class_time'))" :disabled="disabledObj['class_time_isDisabled']"></el-input>
				...
				<el-form-item label="课程简介" prop="course_introduction">
						...
						v-if="user_group === '管理员' || (form['experimental_courses_id'] && $check_field('set','course_introduction')) || (!form['experimental_courses_id'] && $check_field('add','course_introduction'))" :disabled="disabledObj['course_introduction_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','course_introduction')">{{form['course_introduction']}}</div>
							</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','facilities') || $check_field('add','facilities') || $check_field('set','facilities')" :xs="24" :sm="24" :lg="24" class="el_form_editor_warp">
				<el-form-item label="设施情况" prop="facilities">
					<quill-editor v-model.number="form['facilities']"
						v-if="user_group === '管理员' || (form['experimental_courses_id'] && $check_field('set','facilities')) || (!form['experimental_courses_id'] && $check_field('add','facilities')) ">
					</quill-editor>
					<div v-else-if="$check_field('get','facilities')" v-html="form['facilities']"></div>
				</el-form-item>
			</el-col>
...
<script>
	...
	export default {
		...
		data() {
			return {
				field: "experimental_courses_id",
				url_add: "~/api/experimental_courses/add?",
				url_set: "~/api/experimental_courses/set?",
				url_get_obj: "~/api/experimental_courses/get_obj?",
				url_upload: "~/api/experimental_courses/upload?",

				query: {
					"experimental_courses_id": 0,
				},
		...
		methods: {
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/experimental_courses/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/experimental_courses/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/experimental_courses/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/experimental_courses/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/experimental_courses/view','get');
