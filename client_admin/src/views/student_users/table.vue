<template>
	<el-main class="bg table_wrap">
		<el-form label-position="right" :model="query" class="form p_4" label-width="120">
			<el-row>
					...
					<el-form-item>
						...
						<download-excel v-if="$check_option('/student_users/table','import_db')" class="export-excel-wrapper" :data="DetailsForm" :fields="json_fields" name="数据导入表格.xls" >
							<el-button type="success">下载导入文档</el-button>
						</download-excel>
						<el-upload v-if="$check_option('/student_users/table','import_db')" action accept = ".xlsx, .xls" :auto-upload="false" :show-file-list="false" :on-change="handle_import">
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/student_users/table','set') || $check_action('/student_users/view','set') || $check_action('/student_users/view','get') || $check_action('//table','add') || $check_action('//view','add')" >
					...
					if (json.result) {
						...
						_this.$get("~/api/user/get_obj?",form_user,(res)=>{
							if(res.result && res.result.obj){
								...
								_this.$post("~/api/student_users/add?", result[i], function (json, status) {
									if (json.result) {
