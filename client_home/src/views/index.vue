<template>
	<div class="page_root" id="root_demo">
		...
		<div class="warp">
		    <div class="container">
		        <div class="row">
					...
					<!-- 推荐实验课程模块(开始) -->
		<div class="warp" v-if="$check_action('/experimental_courses/list', 'get')">
		    <!-- 容器 -->
		    <div class="container">
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/experimental_courses/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_experimental_courses = !show_list_experimental_courses">
		                        <div class="switch_box">
		                            <span v-if="show_list_experimental_courses"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!-- 实验课程推荐列表组件 -->
		                <list_experimental_courses v-if="show_list_experimental_courses" :list="list_experimental_courses" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_experimental_courses.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_experimental_courses" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_experimental_courses" :key="i" @click="to_details('experimental_courses',o,'experimental_courses_id')">
		                            <td v-for="(oj,n) in fields_experimental_courses">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
		...
		<!-- 推荐实验课程模块(结束) -->
													<!-- 推荐实验室信息模块(开始) -->
		<div class="warp" v-if="$check_action('/laboratory_information/list', 'get')">
		    <!-- 容器 -->
		    <div class="container">
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/laboratory_information/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_laboratory_information = !show_list_laboratory_information">
		                        <div class="switch_box">
		                            <span v-if="show_list_laboratory_information"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!-- 实验室信息推荐列表组件 -->
		                <list_laboratory_information v-if="show_list_laboratory_information" :list="list_laboratory_information" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_laboratory_information.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_laboratory_information" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_laboratory_information" :key="i" @click="to_details('laboratory_information',o,'laboratory_information_id')">
		                            <td v-for="(oj,n) in fields_laboratory_information">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
										<span v-else-if="oj.type && oj.type == 'UID' ">
											<span style="display: none">
...
<script>
	import mixin from "@/mixins/page.js";
				import list_experimental_courses from "@/components/diy/list_experimental_courses.vue";
												import list_laboratory_information from "@/components/diy/list_laboratory_information.vue";
	...
	export default {
		mixins: [mixin],
		components: {
						list_experimental_courses,
														list_laboratory_information,
				bar_title,
			list_article,
			swiper_img,
			swiper_notice,
		...
		data() {
			return {
				...
				show_list_article: true,
							show_list_experimental_courses: true,
				list_experimental_courses: [],
				vm_arr_experimental_courses: [
				...
				fields_experimental_courses: [
										{
						key: "laboratory_diagram",
						label: "实验室图",
						type: "图片"
					},
													{
						key: "laboratory_number",
						label: "实验室编号",
						type: "文本"
					},
													{
						...
						type: "编辑"
															...
															show_list_laboratory_information: true,
				list_laboratory_information: [],
				vm_arr_laboratory_information: [
		...
		methods: {
			...
			toToggle(){
				this.isAdmin = !this.isAdmin;
						...
						// 获取实验课程列表
			get_experimental_courses() {
				let url = "~/api/experimental_courses/get_list?";
										this.$get(url, {
									"page": 1,
					"size": 8
				}, (json) => {
					if (json.result) {
						this.list_experimental_courses = json.result.list;
					}
				})
			},
														// 获取实验室信息列表
			get_laboratory_information() {
				let url = "~/api/laboratory_information/get_list?";
										this.$get(url, {
									"page": 1,
					"size": 8
				}, (json) => {
					if (json.result) {
						this.list_laboratory_information = json.result.list;
					}
				})
			},
		...
		mounted() {
						this.get_experimental_courses();
														this.get_laboratory_information();
				this.get_menu();
			this.get_slides();
			this.get_article();
			// this.get_link();
		...
		computed: {
					    list_table_laboratory_information() {
		        var list = this.list_experimental_courses;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_experimental_courses.map((o) => {
		                // 第二个中括号是对象的属性
		        ...
		        return list_table;
													    ...
													    list_table_laboratory_information() {
		        var list = this.list_laboratory_information;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_laboratory_information.map((o) => {
		                // 第二个中括号是对象的属性
		                list_table[i][o] = list[i][o] || "";
		            });
		        }
