<template>
	<div class="diy_details page_experimental_courses" id="experimental_courses_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_experimental_courses'>
							<div_experimental_courses :obj="obj"></div_experimental_courses>
...
<script>
	import div_experimental_courses from "@/components/diy/div_experimental_courses.vue";
	...
	export default {
		...
		data(){
			return{
				...
				list_comment: [],
				form_comment: {
					source_table: "experimental_courses",
					source_field: "experimental_courses_id",
					source_id: 0,
					reply_to_id: 0,
				},
				count_pages: 1, // 页面数
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/experimental_courses/set?experimental_courses_id=' + obj["experimental_courses_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "experimental_courses",
							source_field: "experimental_courses_id",
							source_id: this.obj.experimental_courses_id,
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.experimental_courses_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "experimental_courses",
					source_field: "experimental_courses_id",
					source_id: obj["experimental_courses_id"],
					user_id
				};
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					this.state_collect = true;
										query.img = this.obj.laboratory_diagram
