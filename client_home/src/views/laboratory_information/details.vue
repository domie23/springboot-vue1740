<template>
	<div class="diy_details page_laboratory_information" id="laboratory_information_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_laboratory_information'>
							<div_laboratory_information :obj="obj"></div_laboratory_information>
...
<script>
	import div_laboratory_information from "@/components/diy/div_laboratory_information.vue";
	...
	export default {
		...
		data(){
			return{
				...
				list_comment: [],
				form_comment: {
					source_table: "laboratory_information",
					source_field: "laboratory_information_id",
					source_id: 0,
					reply_to_id: 0,
				},
				count_pages: 1, // 页面数
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/laboratory_information/set?laboratory_information_id=' + obj["laboratory_information_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "laboratory_information",
							source_field: "laboratory_information_id",
							source_id: this.obj.laboratory_information_id,
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.laboratory_information_id;
				}
			},
			/**
			 * 获取点赞
			...
			get_praise(obj) {
				...
				this.$get("~/api/praise/count?", {
					source_table: "laboratory_information",
					source_field: "laboratory_information_id",
					source_id: obj["laboratory_information_id"]
				}, (res) => {
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/laboratory_information/set?laboratory_information_id=' + _this.obj["laboratory_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){
